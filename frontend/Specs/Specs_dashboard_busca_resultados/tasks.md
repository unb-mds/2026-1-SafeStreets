...
- [x] T001 [P] Estender `Noticia` em `utils/noticias.ts` com `risco: "Alto" |
  "Médio" | "Baixo"`, `lat: number`, `lng: number` (centróide aproximado de cada
  região) para os 6 itens mockados — `frontend/utils/noticias.ts`
- [x] T002 Criar `algumFiltroAplicado` e `filtrarNoticias` em `utils/filtros.ts`
  ...
  (depende: T001)
- [x] T003 [P] Testes de `filtrarNoticias`/`algumFiltroAplicado`: região isolada,
...
- [x] T004 [P] Criar `gerarResumoIA(noticia)` em `utils/iaResumo.ts`: mock
  ...
  T001)
- [x] T005 [P] Teste de `gerarResumoIA`: resolve para notícia comum; rejeita para
  `id "5"` — `frontend/__tests__/utils/iaResumo.test.ts` (depende: T004)

## Fase 1 — Componente `ResultadoBusca`
- [x] T006 Teste de `ResultadoBusca`: renderiza título, `RA-XXXXX` e região;
...
- [x] T007 Implementar `ResultadoBusca.tsx` + `.module.css` (botão de lista,
...
- [x] T011 Atualizar `PainelFiltros.tsx`: prop `onSelecionarNoticia`, computa
...
- [x] T012 Estender teste de `MapaInterativo`/`MapView` (mock de `react-leaflet`
